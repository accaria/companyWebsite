  ...
  return (
    <>
      <div className="w-full h-full p-[50px] flex flex-col justify-center items-start">
        ...
        <p>ゲームスクエアの社風での優秀なメンバーをこちらでご紹介致します！</p>
      </div>
      <div className="flex flex-col justify-center items-center gap-10">
        ...
        <div className="w-full flex flex-wrap justify-center items-center gap-6 lg:gap-30">
          {firstDiv.map((member, index) => {
            const randomRole =
              jobRole[Math.floor(Math.random() * jobRole.length)];
            return (
              <div
                className="flex flex-col justify-center border-orange-400 border-4 text-center items-center max-w-xs"
                key={index}
              >
